...
**Workaround confirmed for the invoke call itself (does not fully unblock R2.1):** Azure ML Studio's "Create job" flow on the batch endpoint's detail page successfully invoked the endpoint — the browser session bypasses whatever `az ml batch-endpoint invoke` hits, producing a real job (`batchjob-d8fb1362-a35b-cfdf-dac4-cd266edd6f9c`) rather than a 403. This matches a previously-documented workaround for the identical bug in the sibling `azure-mlops` project (`.serena/memories/known_bugs.md`, "P2 — Resolve local AML CLI tenant mismatch," dated 2026-08-10, predating this session by a week — "Azure ML Studio is the current workaround"). The job itself then failed for an unrelated, separately-tracked reason (`AML-DATA-IDENTITY-01`, see below) — so this confirms the invoke-layer bug has a working manual bypass, but does not itself unblock R2.7/R2.9's governed automation, since Studio's UI isn't scriptable from a GitHub Actions workflow.
...
### AML-DATA-IDENTITY-01 — revised scope: trigger is fresh credential resolution, not content novelty

Same-day follow-up testing corrected the original scope claim above ("new content fails, previously-uploaded content is unaffected"). Two further tests, both after the diagnostic key-toggle had already been reverted (`allowSharedKeyAccess: False` confirmed):

- A Studio-invoked batch job (see the R2.1 workaround below) against `azure-ai-ml-ops-batch-data:1` — a `LocalUpload/`-prefixed blob untouched since before this investigation began — **failed identically** (`batchjob-d8fb1362-a35b-cfdf-dac4-cd266edd6f9c` / child `4b549f23-c6e1-44cc-81f2-55ecc6f7e367`, ~15:50:18Z).
- A re-run of the training pipeline against `azure-ai-ml-ops-training-data:6` — the exact blob this project's original successful `dreamy_chain_xwc3x25ykn` run used — **also failed identically** (`calm_evening_dqnpm9qtvd` / child `96878eba-2460-4ce8-803c-550cacf483d2`, ~16:0xZ).

Cross-referenced against Azure Monitor's storage-account `Transactions` metric (dimensioned by `ResponseType`, no diagnostic settings required): `AuthenticationError` counts were **zero for all of 2026-08-13 through 2026-08-15** — the entire span covering this project's original successful proof and this session's earlier R2.1–R2.3 work — then began the same hour this investigation's active mount-failure testing started (`04:00Z: 1`, `06:00Z: 8`, `15:00Z: 1`, `16:00Z: 1`, matching the diagnostic runs above almost exactly).

**Revised conclusion:** the defect is real and unchanged (`rslex` defaults to key-based SAS on fresh credential resolution regardless of RBAC), but the trigger is *fresh resolution*, not *new content specifically*. Content that has never mounted before always needs fresh resolution (why R2.4's very first attempt failed before any account setting was touched). Content that had been mounting successfully for days can be forced back into needing fresh resolution by an unrelated account-level security-setting change — in this case, this investigation's own `allowSharedKeyAccess` toggle (on, then off again) — which is why previously-stable assets failed only after that toggle, not before. Both symptoms trace to the same underlying bug.

A parallel hypothesis — that a Microsoft Entra Suite license added to this tenant on 2026-08-09 (bundling Identity Protection/P2, Global Secure Access, Identity Governance) was the cause, given its topical similarity to the separate `AML-BATCH-403-TENANT-MISMATCH` issue — was investigated and ruled out. Conditional Access policies, cross-tenant B2B settings, cross-tenant synchronization, Identity Protection risk policies, Global Secure Access Tenant Restrictions, and PIM-eligible (vs. standing) role assignments were all checked directly and found empty or inapplicable. The metrics timeline is also inconsistent with an Aug-9-dated cause: five full days of zero authentication errors separate the license addition from this investigation's testing.

**Verified by:** Ray Swan / Claude, `az monitor metrics list` against the storage account's blob-service resource, cross-referenced with job timestamps from `az ml job show`/`az storage blob download` on the two new diagnostic runs.
